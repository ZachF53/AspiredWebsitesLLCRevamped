...
@shared_task
def run_dunning_sweep_task():
    """Daily — advance every open payment-failure window by what is due.

    This replaced the nine `apply_async(countdown=...)` messages the
    `invoice.payment_failed` webhook used to queue. Everything about why
    is in billing/dunning.py; the short version is that scheduling work
    weeks ahead meant acting on stale state, and Redis redelivered the
    parked messages until prod held 42 copies of each.
    """
    from billing.dunning import run_dunning_sweep

    summary = run_dunning_sweep()
    logger.info('run_dunning_sweep_task: %s', summary)
    return summary
